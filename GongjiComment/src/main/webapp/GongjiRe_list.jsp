...
<%@page import="Service.GongjiReServiceImpl"%>
<%@page import="Service.GongjiReService"%>
...
<%
	try {
		...
		List<GongjiRe> gongjiRes = gongjiReDao.selectAll(pageNum, countPerPage);
	
		// StudentScoreService 인터페이스의 구현 클래스의 인스턴스를 생성합니다.
		GongjiReService gongjiReService = new GongjiReServiceImpl();
	
		// studentScoreService 인스턴스의 getPagination() 메서드를 호출하여 Pagination 객체를 가져옵니다.
		// pageNum과 countPerPage 매개변수는 페이징 정보를 계산하는 데 사용됩니다.
		Pagination pagination = gongjiReService.getPagination(pageNum, countPerPage);
...
<body>
	...
	<table cellspacing=1 width=800 border=0>
		...
		<td><p style="font-size: 30px; font-weight: bold; text-align: center;">분당 융기원 공지사항</p></td>
		<!-- 테이블 셀 안에 제목을 출력합니다. -->
	</table>
	<br>
	<table cellspacing=1 width=800 border=1>
		...
		for (int i = 0; i < gongjiRes.size(); i++) {
			// 공지사항 목록의 크기만큼 반복합니다.
			GongjiRe gongjiRe = gongjiRes.get(i);
			String str = ""; // str 변수 생성
			if (gongjiRe.getDate().equals(formatedNow)) str = " [NEW]"; // 새 게시물이면 new
			
			out.println("<tr>");
			// 테이블 행을 시작합니다.
			out.println("<td width=50><p align=center>" + gongjiRe.getId() + "</p></td>");
			// 번호를 출력합니다.
			if(gongjiRe.getReLevel() == 0) {
				String title = gongjiRe.getTitle().replaceAll("&", "&amp;"); // &를 &amp로 변경
		    	String title2 = title.replaceAll("<", "&lt;").replaceAll(">", "&gt;"); // <와 >를 각각 &lt, &gt로 변경
		    	out.println("<td width=400><p align=left><a href='GongjiRe_view.jsp?key=" + gongjiRe.getId() + "'>" + title2 + str + "</a></p></td>");
			} else {
				out.println("<td width=400><p align=left><a href='GongjiRe_view.jsp?key=" + gongjiRe.getId() + "'>");
				
				for (int j = 0; j < gongjiRe.getReLevel(); j++) { // relevel만큼 화살표 출력
					out.print("↳ ");
				}
				out.print("[RE]"); // RE 붙이기
				...
				out.println("</a></p></td>"); // td 닫기
			}
			
			out.println("<td width=150><p align=center>" + gongjiRe.getViewCnt() + "</p></td>");
			// 조회수을 출력합니다.
			out.println("<td width=150><p align=center>" + gongjiRe.getDate() + "</p></td>");
			// 등록일을 출력합니다.
			out.println("</tr>");
			// 테이블 행을 닫습니다.
		}
	...
	<table width=800>
		...
		<tr>
			...
			<td><input type=button value="신규"
				OnClick="window.location='GongjiRe_insert.jsp'"></td>
	...
	<div class="bottom_paging_box" style="width: 800px;">
		...
		<a href="GongjiRe_list.jsp?page=<%=pp%>"><<</a> <a href="GongjiRe_list.jsp?page=<%=p%>"><</a>
		<% } %>
		...
		<a style="color: red;" href="GongjiRe_list.jsp?page=<%=i%>"><%=i%></a>
		<% } else { %>
		<!-- 다른 페이지를 표시하는 링크를 생성합니다. -->
		<a href="GongjiRe_list.jsp?page=<%=i%>"><%=i%></a>
		<% } %>
		<% } %>
		...
		<a href="GongjiRe_list.jsp?page=<%=n%>">></a> <a href="GongjiRe_list.jsp?page=<%=nn%>">>></a>
		<% } %>
	</div>
