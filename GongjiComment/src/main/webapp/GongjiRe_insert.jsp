...
<body>
	<%
	java.sql.Date Todaydate = new java.sql.Date(System.currentTimeMillis()); // 현재 날짜를 가져옵니다.
	...
	<form method=post name='fm'>
		<table width=800 border=1 cellspacing=0 cellpadding=5>
			...
			<tr>
				<td><b>제목</b></td> <!-- 제목 열 -->
				<td><input type=text name=title size=66 maxlength=50></td> <!-- 제목 입력 필드 -->
			</tr>
			<tr>
				<td><b>일자</b></td> <!-- 일자 열 -->
				<td><%=Todaydate%></td> <!-- 현재 날짜 출력 -->
			</tr>
			<tr>
				<td><b>내용</b></td> <!-- 내용 열 -->
			...
			int rootId = gongjiReDao.getNewId(); // rootid 받아오기
			
			int reCnt = 0; // recnt 지정
			
			%>
			<input type=hidden name=rootId value='<%=rootId%>'> <!--rootid-->
			<input type=hidden name=reLevel value='<%=0%>'> <!--relevel-->
		...
		<table width=800>
			<tr>
				<td width=700></td>
				<td><input type=button value="취소" OnClick="history.back()"></td> <!-- 취소 버튼 -->
